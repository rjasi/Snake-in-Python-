...
#Version : 2                   #
################################
#SimpleGraphics library is owned by Ben Stepenson and not me
...
Need to fix a bug where when a segment is added, the new segment is being draw when functions are called in a certain
order 
"""
...
STARTING_SEG = 5 #Starting number of segments that the snake has 
...
class GameEngine:
	...
	def __init__ (self):
		self.segments = STARTING_SEG #Number of starting segments (Must make a variable for this instand of hard coding it)
	...
	"""need to rework this function and how it handles when the coin is caught"""	
	def move_snake (self):
		if self.caught_coin() == False: #check if the new position of the snake caught the coin 
			if self.snake_crashed() == True:
				return True #Check if the new position of the snake is invalid (out of bounds or crashed into itself)
	...
	def caught_coin (self):
		if (self.snk[0].pos_x + self.nextpos.pos_x == self.coin.pos_x) and (self.snk[0].pos_y + self.nextpos.pos_y == self.coin.pos_y):
	...
	def snake_crashed (self):
		...
		if (self.snk[0].pos_x + self.nextpos.pos_x) > MAXX or \
			(self.snk[0].pos_x + self.nextpos.pos_x) < MINX  or \
			(self.snk[0].pos_y + self.nextpos.pos_y > MAXY) or \
			(self.snk[0].pos_y + self.nextpos.pos_y < MINY):
			print ("You are out of bounds!")
			return True

		#check if the head of snake crashed into itself
		for i in range (1,self.segments):
			if self.snk[0].pos_x + self.nextpos.pos_x == self.snk[i].pos_x and \
				self.snk[0].pos_y + self.nextpos.pos_y == self.snk[i].pos_y:
